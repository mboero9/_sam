  ...
  $.fn.multifilter = function(options) {
    ...
    this.each(function() {
      ...
      if (settings.method === 'thead') {
        ...
        var filtro = container.find('th:Contains(' + $this.data('filtro') + ')');
        var col_index = container.find($('thead th')).index(filtro);
      };

      if (settings.method === 'class') {
        // Match the data-col attribute to the class on each column
        var filtro = rows.first().find('td.' + $this.data('filtro') + '');
        var col_index = rows.first().find('td').index(filtro);
      };
